...
raw_data = pd.read_excel('owid-covid-data.xlsx',sheet_name ='Sheet1')

# User Prompts
# location = input("Choose your country of choice")
location = 'Bahamas'
...
# Filtered Data
filtered_data = raw_data.loc[(raw_data['location'] == location) & (raw_data['date']>=start_date) & (raw_data['date']<=end_date)]
filtered_data
print(filtered_data)
print('------')
print(filtered_data[['date','total_cases']])
print('------')
...
def chart():
    legend = 'Monthly Data'
    labels = filtered_data['date']
    values = filtered_data['total_cases']
    # trial_data = filtered_data[['date','total_cases']].reset_index()
    # return filtered_data[['date','total_cases']].to_json(orient='records')
